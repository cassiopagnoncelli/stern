...
module Stern
  ...
  class WithdrawFeeServicePayRevert < BaseOperation
    ...
    def perform_undo
      ...
      if EntryPair.find_by(code: ENTRY_PAIRS[:add_withdraw_fee_service_pay_revert_usd], uid: withdraw_id).present?
        EntryPair.remove_withdraw_fee_service_pay_revert_usd(withdraw_id)
      end
    end
  end
end
