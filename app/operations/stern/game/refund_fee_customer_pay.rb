...
module Stern
  # Refund fee is paid by the customer.
  class RefundFeeCustomerPay < BaseOperation
    ...
    def perform(operation_id)
      ...
      EntryPair.add_refund_fee_customer_pay_usd(refund_fcp_id, customer_id, fee, nil, operation_id:) if fee.present?
    end

    def perform_undo
      raise ArgumentError if invalid?(:undo)

      if EntryPair.find_by(code: ENTRY_PAIRS[:add_refund_fee_customer_pay_usd], uid: refund_fcp_id).present?
        EntryPair.remove_refund_fee_customer_pay_usd(refund_fcp_id)
      end
    end
  end
end
