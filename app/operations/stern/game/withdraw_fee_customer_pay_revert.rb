...
module Stern
  ...
  class WithdrawFeeCustomerPayRevert < BaseOperation
    ...
    def perform(operation_id)
      ...
      EntryPair.add_withdraw_fee_customer_pay_revert_usd(withdraw_fcp_id, customer_id, fee, nil, operation_id:) if fee.present?
    end

    def perform_undo
      raise ArgumentError if invalid?(:undo)

      if EntryPair.find_by(code: ENTRY_PAIRS[:add_withdraw_fee_customer_pay_revert_usd], uid: withdraw_fcp_id).present?
        EntryPair.remove_withdraw_fee_customer_pay_revert_usd(withdraw_fcp_id)
      end
    end
  end
end
