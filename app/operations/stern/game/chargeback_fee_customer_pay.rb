...
module Stern
  # Chargeback fee is paid by the customer.
  class ChargebackFeeCustomerPay < BaseOperation
    ...
    def perform(operation_id)
      ...
      EntryPair.add_chargeback_fee_customer_pay_usd(chargeback_fcp_id, customer_id, fee, nil, operation_id:) if fee.present?
    end

    def perform_undo
      raise ArgumentError if invalid?(:undo)

      if EntryPair.find_by(code: ENTRY_PAIRS[:add_chargeback_fee_customer_pay_usd], uid: chargeback_fcp_id).present?
        EntryPair.remove_chargeback_fee_customer_pay_usd(chargeback_fcp_id)
      end
    end
  end
end
