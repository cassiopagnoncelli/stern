...
module Stern
  # Unlock bonus in customer's account.
  class BonusUnlock < BaseOperation
    ...
    # @param amount [Bigint] amount unlocked from locked bonus balance
    ...
    def perform(operation_id)
      ...
      EntryPair.add_customer_bonus_unlock_usd(bonus_unlock_id, customer_id, amount, nil, operation_id:) if amount.present?
    end

    def perform_undo
      raise ArgumentError if invalid?(:undo)

      if EntryPair.find_by(code: ENTRY_PAIRS[:add_customer_bonus_unlock_usd], uid: bonus_unlock_id).present?
        EntryPair.remove_customer_bonus_unlock_usd(bonus_unlock_id)
      end
    end
  end
end
