...
module Stern
  class DepositFeeServicePayRevert < BaseOperation
    ...
    def perform(operation_id)
      ...
      EntryPair.add_service_deposit_fee_revert_usd(deposit_id, customer_id, fee, nil, operation_id:) if fee.present?
    end

    def perform_undo
      raise ArgumentError if invalid?(:undo)

      if EntryPair.find_by(code: ENTRY_PAIRS[:add_service_deposit_fee_revert_usd], uid: deposit_id).present?
        EntryPair.remove_service_deposit_fee_revert_usd(deposit_id)
      end
    end
  end
end
