...
module Stern
  ...
  class WithdrawFeeServicePay < BaseOperation
    ...
    # @param fee [Bigint] fee paid by the service
    ...
    def perform(operation_id)
      ...
      EntryPair.add_withdraw_fee_service_pay_usd(withdraw_fsp_id, customer_id, fee, nil, operation_id:) if fee.present?
    end

    def perform_undo
      raise ArgumentError if invalid?(:undo)

      if EntryPair.find_by(code: ENTRY_PAIRS[:add_withdraw_fee_service_pay_usd], uid: withdraw_fsp_id).present?
        EntryPair.remove_withdraw_fee_service_pay_usd(withdraw_fsp_id)
      end
    end
  end
end
