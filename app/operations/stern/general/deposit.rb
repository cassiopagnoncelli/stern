...
module Stern
  class Deposit < BaseOperation
    ...
    # @param amount [Bigint] amount added to customer balance
    ...
    def perform(operation_id)
      ...
      EntryPair.add_customer_deposit_usd(deposit_id, customer_id, amount, nil, operation_id:) if amount.present?
    end

    def perform_undo
      raise ArgumentError if invalid?(:undo)

      if EntryPair.find_by(code: ENTRY_PAIRS[:add_customer_deposit_usd], uid: deposit_id).present?
        EntryPair.remove_customer_deposit_usd(deposit_id)
      end
    end
  end
end
