...
module Stern
  # Revert a put option.
  class PutOptionRevert < BaseOperation
    ...
    def perform(operation_id)
      ...
      EntryPair.add_put_option_revert_usd(put_option_id, customer_id, amount, nil, operation_id:) if amount.present?
    end

    def perform_undo
      raise ArgumentError if invalid?(:undo)

      if EntryPair.find_by(code: ENTRY_PAIRS[:add_put_option_revert_usd], uid: put_option_id).present?
        EntryPair.remove_put_option_revert_usd(put_option_id)
      end
    end
  end
end
