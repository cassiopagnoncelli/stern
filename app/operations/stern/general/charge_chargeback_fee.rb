...
module Stern
  # Charges a chargeback fee against a stakeholder's `*_available`, optionally
  # drawing from `*_credit` first to reduce out-of-pocket.
  #
  # A negative `amount` reverses a previously-charged fee — the same entry
  # pair is emitted with flipped signs. Reverse-chargeback callers rely on
  # this idiom because `ReverseChargeback` only unwinds the underlying fund
  # movement, not the fee. Credit application is intentionally skipped on
  # the reversal path: the credit redemption from the original charge is its
  # own movement and is not undone here.
  class ChargeChargebackFee < BaseOperation
    ...
    performs_stakeholder_pair "charge_chargeback_fee_%{type}",
