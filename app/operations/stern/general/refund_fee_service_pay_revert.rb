...
module Stern
  # Revert of refund fee paid by the service.
  class RefundFeeServicePayRevert < BaseOperation
    ...
    def perform(operation_id)
      ...
      EntryPair.add_refund_fee_service_pay_revert_usd(refund_fsp_id, customer_id, fee, nil, operation_id:) if fee.present?
    end

    def perform_undo
      raise ArgumentError if invalid?(:undo)

      if EntryPair.find_by(code: ENTRY_PAIRS[:add_refund_fee_service_pay_revert_usd], uid: refund_fsp_id).present?
        EntryPair.remove_refund_fee_service_pay_revert_usd(refund_fsp_id)
      end
    end
  end
end
