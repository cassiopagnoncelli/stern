...
module Stern
  class DepositFeeServicePay < BaseOperation
    ...
    def perform(operation_id)
      ...
      EntryPair.add_service_deposit_fee_usd(deposit_fsp_id, customer_id, fee, nil, operation_id:) if fee.present?
    end

    def perform_undo
      raise ArgumentError if invalid?(:undo)

      if EntryPair.find_by(code: ENTRY_PAIRS[:add_service_deposit_fee_usd], uid: deposit_fsp_id).present?
        EntryPair.remove_service_deposit_fee_usd(deposit_fsp_id)
      end
    end
  end
end
