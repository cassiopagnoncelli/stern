  ...
  class ChargePaymentFee < BaseOperation
    ...
    def target_tuples
      if merchant_id.present?
        tuples_for_pair("charge_#{method}_fee_merchant".to_sym, merchant_id, merchant_id, currency)
      elsif customer_id.present?
        tuples_for_pair("charge_#{method}_fee_customer".to_sym, customer_id, customer_id, currency)
      elsif partner_id.present?
        tuples_for_pair("charge_#{method}_fee_partner".to_sym, partner_id, partner_id, currency)
      end
    end
    ...
    def perform(operation_id)
      ...
      if merchant_id.present?
        EntryPair.public_send("add_charge_#{method}_fee_merchant".to_sym, merchant_id, merchant_id, fee, currency, operation_id:)
      elsif customer_id.present?
        EntryPair.public_send("add_charge_#{method}_fee_customer".to_sym, customer_id, customer_id, amount, currency, operation_id:)
      elsif partner_id.present?
        EntryPair.public_send("add_charge_#{method}_fee_partner".to_sym, partner_id, partner_id, amount, currency, operation_id:)
      end
    end
  end
end
