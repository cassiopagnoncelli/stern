...
module Stern
  ...
  class RefundRevert < BaseOperation
    ...
    def perform(operation_id)
      ...
      EntryPair.add_customer_refund_revert_usd(refund_id, customer_id, amount, nil, operation_id:) if amount.present?
    end

    def perform_undo
      raise ArgumentError if invalid?(:undo)

      if EntryPair.find_by(code: ENTRY_PAIRS[:add_customer_refund_revert_usd], uid: refund_id).present?
        EntryPair.remove_customer_refund_revert_usd(refund_id)
      end
    end
  end
end
