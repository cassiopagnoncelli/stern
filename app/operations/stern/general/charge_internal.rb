...
module Stern
  class ChargeInternal < BaseOperation
    ...
    def target_tuples
      tuples = tuples_for_pair(:pp_charge_internal, merchant_id, currency)
      tuples += tuples_for_pair(:pp_charge_fee_merchant_internal, merchant_id, currency) if fee&.positive?
    ...
    def perform(operation_id)
      ...
      EntryPair.add_pp_charge_internal(charge_id, merchant_id, amount, currency, operation_id:)
      EntryPair.add_pp_charge_fee_merchant_internal(charge_id, merchant_id, fee, currency, operation_id:) if fee&.positive?
      if customer_id
