...
module Stern
  class ChargeBankTransfer < BaseOperation
    ...
    def target_tuples
      tuples = tuples_for_pair(:pp_charge_bank_transfer, merchant_id, currency)
      tuples += tuples_for_pair(:pp_charge_fee_merchant_bank_transfer, merchant_id, currency) if fee&.positive?
    ...
    def perform(operation_id)
      ...
      EntryPair.add_pp_charge_bank_transfer(charge_id, merchant_id, amount, currency, operation_id:)
      EntryPair.add_pp_charge_fee_merchant_bank_transfer(charge_id, merchant_id, fee, currency, operation_id:) if fee&.positive?
      if customer_id
