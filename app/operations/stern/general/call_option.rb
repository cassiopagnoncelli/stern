...
module Stern
  # Place a call option.
  class CallOption < BaseOperation
    ...
    def perform(operation_id)
      ...
      EntryPair.add_call_option_usd(call_option_id, customer_id, amount, nil, operation_id:) if amount.present?
    end

    def perform_undo
      raise ArgumentError if invalid?(:undo)

      if EntryPair.find_by(code: ENTRY_PAIRS[:add_call_option_usd], uid: call_option_id).present?
        EntryPair.remove_call_option_usd(call_option_id)
      end
    end
  end
end
