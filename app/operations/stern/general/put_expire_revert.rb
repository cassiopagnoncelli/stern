...
module Stern
  # Revert a put option expire.
  class PutExpireRevert < BaseOperation
    ...
    # @param amount [Bigint] amount reverted from the put option
    ...
    def perform(operation_id)
      ...
      EntryPair.add_put_expire_revert_usd(put_option_id, customer_id, amount, nil, operation_id:) if amount.present?
    end

    def perform_undo
      raise ArgumentError if invalid?(:undo)

      if EntryPair.find_by(code: ENTRY_PAIRS[:add_put_expire_revert_usd], uid: put_option_id).present?
        EntryPair.remove_put_expire_revert_usd(put_option_id)
      end
    end
  end
end
