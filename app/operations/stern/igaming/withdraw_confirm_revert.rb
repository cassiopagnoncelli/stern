...
module Stern
  # Revert withdraw confirm.
  class WithdrawConfirmRevert < BaseOperation
    ...
    def perform(operation_id)
      ...
      EntryPair.add_withdraw_confirm_revert_usd(withdraw_id, customer_id, amount, nil, operation_id:) if amount.present?
    end

    def perform_undo
      raise ArgumentError if invalid?(:undo)

      if EntryPair.find_by(code: ENTRY_PAIRS[:add_withdraw_confirm_revert_usd], uid: withdraw_id).present?
        EntryPair.remove_withdraw_confirm_revert_usd(withdraw_id)
      end
    end
  end
end
