...
module Stern
  ...
  class WithdrawFeeServicePay < BaseOperation
    ...
    # @param amount [Bigint] amount paid by the service
    ...
    def perform(operation_id)
      ...
      EntryPair.add_withdraw_fee_service_pay_usd(withdraw_id, customer_id, amount, nil, operation_id:) if amount.present?
    end

    def perform_undo
      raise ArgumentError if invalid?(:undo)

      if EntryPair.find_by(code: ENTRY_PAIRS[:add_withdraw_fee_service_pay_usd], uid: withdraw_id).present?
        EntryPair.remove_withdraw_fee_service_pay_usd(withdraw_id)
      end
    end
  end
end
