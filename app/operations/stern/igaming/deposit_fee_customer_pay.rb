...
module Stern
  class DepositFeeCustomerPay < BaseOperation
    ...
    def perform(operation_id)
      ...
      EntryPair.add_customer_deposit_fee_usd(deposit_id, customer_id, fee, nil, operation_id:) if fee.present?
    end

    def perform_undo
      raise ArgumentError if invalid?(:undo)

      if EntryPair.find_by(code: ENTRY_PAIRS[:add_customer_deposit_fee_usd], uid: deposit_id).present?
        EntryPair.remove_customer_deposit_fee_usd(deposit_id)
      end
    end
  end
end
