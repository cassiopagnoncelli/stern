...
module Stern
  # Give bonus to customer's igaming account.
  class BonusGive < BaseOperation
    ...
    def perform(operation_id)
      ...
      EntryPair.add_customer_give_bonus_locked_usd(bonus_id, customer_id, amount, nil, operation_id:) if amount.present?
    end

    def perform_undo
      raise ArgumentError if invalid?(:undo)

      if EntryPair.find_by(code: ENTRY_PAIRS[:add_customer_give_bonus_locked_usd], uid: bonus_id).present?
        EntryPair.remove_customer_give_bonus_locked_usd(bonus_id)
      end
    end
  end
end
