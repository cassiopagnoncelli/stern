  ...
  class PayPix < BaseOperation
    ...
    def perform(operation_id)
      ...
      if charged_fees.abs.positive?
        EntryPair.add_pix_fee(payment_id, merchant_id, charged_fees, operation_id:)
      end
      EntryPair.add_pix_payment(payment_id, merchant_id, amount, credit_entry_pair_id, operation_id:)
    end

    def perform_undo
      raise ArgumentError if invalid?(:undo)

      credit_entry_pair_id = EntryPair.find_by!(code: ENTRY_PAIRS[:add_pix_payment], uid: payment_id).credit_entry_pair_id
      EntryPair.remove_credit(credit_entry_pair_id) if credit_entry_pair_id.present?
      EntryPair.remove_pix_fee(payment_id)
      EntryPair.remove_pix_payment(payment_id)
    end
  end
end
