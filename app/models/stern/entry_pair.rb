...
module Stern
  class EntryPair < ApplicationRecord
    ...
    validates :uid, presence: true, uniqueness: { scope: [ :code, :currency ] }
    ...
    def self.double_entry_add(code, gid, uid, book_add, book_sub, amount, currency, timestamp, operation_id)
      ...
      Entry.create!(book_id: Book.code(book_add), gid:, entry_pair_id: entry_pair.id, amount:, currency:, timestamp:)
      Entry.create!(book_id: Book.code(book_sub), gid:, entry_pair_id: entry_pair.id, amount: -amount, currency:, timestamp:)
      entry_pair.id
    end
