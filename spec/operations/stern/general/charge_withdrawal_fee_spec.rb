...
module Stern
  RSpec.describe ChargeWithdrawalFee, type: :model do
    ...
    describe "#target_tuples" do
      ...
      it "partner: locks fee pair and credit pair tuples" do
        ...
        expect(op.target_tuples).to eq([
          ...
          [ "wdw_partner_fee",   partner_id, "BRL" ],
    ...
    describe "#call" do
      ...
      it "debits merchant_available and credits wdw_merchant_fee by amount" do
        described_class.new(**valid_inputs(amount: 100)).call

        expect(::Stern.balance(merchant_id, :merchant_available, :BRL)).to eq(-100)
        expect(::Stern.balance(merchant_id, :wdw_merchant_fee, :BRL)).to eq(100)
      end

      it "writes one entry pair (charge_withdrawal_fee_merchant) keyed by merchant_id" do
        described_class.new(**valid_inputs(amount: 100)).call

        expect(EntryPair.last).to have_attributes(
          code: "charge_withdrawal_fee_merchant",
          uid: merchant_id,
          amount: 100,
      ...
      it "writes charge_withdrawal_fee_customer for the customer variant" do
        described_class.new(**valid_inputs(merchant_id: nil, customer_id:, amount: 100)).call
        expect(EntryPair.last.code).to eq("charge_withdrawal_fee_customer")
      end

      it "writes charge_withdrawal_fee_partner for the partner variant" do
        described_class.new(**valid_inputs(merchant_id: nil, partner_id:, amount: 100)).call
        expect(EntryPair.last.code).to eq("charge_withdrawal_fee_partner")
      end
      ...
      context "with available credit" do
        ...
        it "applies customer credit for the customer variant" do
          ...
          expect(::Stern.balance(customer_id, :wdw_customer_fee, :BRL)).to eq(100)
        end
        ...
        it "applies partner credit for the partner variant" do
          ...
          expect(::Stern.balance(partner_id, :wdw_partner_fee, :BRL)).to eq(100)
        end
