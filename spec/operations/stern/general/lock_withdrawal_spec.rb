...
module Stern
  RSpec.describe LockWithdrawal, type: :model do
    ...
    describe "#target_tuples" do
      it "merchant: pins merchant_id to lock_withdrawal_merchant pair's two books" do
      ...
      it "customer: pins customer_id to lock_withdrawal_customer pair's two books" do
        op = described_class.new(**valid_inputs(merchant_id: nil, customer_id:))
        expect(op.target_tuples).to eq([
      ...
      it "partner: pins partner_id to lock_withdrawal_partner pair's two books" do
        op = described_class.new(**valid_inputs(merchant_id: nil, partner_id:))
        expect(op.target_tuples).to eq([
    ...
    describe "#call" do
      ...
      it "moves balance from merchant_available to wdw_merchant_outbound" do
        ...
        expect(::Stern.balance(merchant_id, :wdw_merchant_outbound, :BRL)).to eq(5000)
      end

      it "writes one entry pair (lock_withdrawal_merchant) keyed by merchant_id" do
        seed_available({ merchant_id: }, amount: 10_000)

        described_class.new(**valid_inputs(amount: 5000)).call

        expect(EntryPair.last).to have_attributes(
          code: "lock_withdrawal_merchant",
          uid: merchant_id,
          amount: 5000,
      ...
      it "writes lock_withdrawal_customer for the customer variant" do
        seed_available({ customer_id: }, amount: 10_000)
        described_class.new(**valid_inputs(merchant_id: nil, customer_id:, amount: 5000)).call
        expect(EntryPair.last.code).to eq("lock_withdrawal_customer")
      end

      it "writes lock_withdrawal_partner for the partner variant" do
