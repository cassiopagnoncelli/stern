...
module Stern
  RSpec.describe WithholdBalance, type: :model do
    ...
    describe "#target_tuples" do
      it "merchant: pins merchant_id to withhold_merchant_balance pair's two books" do
      ...
      it "customer: pins customer_id to withhold_customer_balance pair's two books" do
        op = described_class.new(**valid_inputs(merchant_id: nil, customer_id:))
        expect(op.target_tuples).to eq([
      ...
      it "partner: pins partner_id to withhold_partner_balance pair's two books" do
        op = described_class.new(**valid_inputs(merchant_id: nil, partner_id:))
        expect(op.target_tuples).to eq([
    ...
    describe "#call" do
      ...
      it "moves balance from merchant_available to merchant_withheld" do
      ...
      it "writes one entry pair (withhold_merchant_balance) keyed by merchant_id" do
        seed_available({ merchant_id: }, amount: 10_000)

        described_class.new(**valid_inputs(amount: 5000)).call

        expect(EntryPair.last).to have_attributes(
          code: "withhold_merchant_balance",
          uid: merchant_id,
          amount: 5000,
      ...
      it "writes withhold_customer_balance for the customer variant" do
        seed_available({ customer_id: }, amount: 10_000)
        described_class.new(**valid_inputs(merchant_id: nil, customer_id:, amount: 5000)).call
        expect(EntryPair.last.code).to eq("withhold_customer_balance")
      end

      it "writes withhold_partner_balance for the partner variant" do
