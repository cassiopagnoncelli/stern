...
module Stern
  RSpec.describe ChargeManual, type: :model do
    ...
    describe "#call" do
      ...
      it "keeps BRL and USD balances fully independent for the same merchant" do
        ...
        brl = ::Stern.balance(merchant_id, :pp_charge_manual, :BRL)
        usd = ::Stern.balance(merchant_id, :pp_charge_manual, :USD)
      ...
      context "with a fee" do
        ...
        it "records the fee in the pp_charge_fee_merchant_manual book" do
          described_class.new(**valid_inputs(charge_id: 11, fee: 100)).call
          fee_balance = ::Stern.balance(merchant_id, :pp_charge_fee_merchant_manual, :BRL)
