...
module Stern
  RSpec.describe ChargeChargebackFee, type: :model do
    ...
    describe "#target_tuples" do
      it "merchant: locks fee pair and credit pair tuples" do
        ...
        expect(op.target_tuples).to eq([
          ...
          [ "chargeback_fee_merchant", merchant_id, "BRL" ],
      ...
      it "partner: locks fee pair and credit pair tuples" do
        ...
        expect(op.target_tuples).to eq([
          ...
          [ "chargeback_fee_partner", partner_id, "BRL" ],
    ...
    describe "#call" do
      ...
      it "debits merchant_available and credits chargeback_fee_merchant by amount" do
        described_class.new(**valid_inputs(amount: 100)).call

        expect(::Stern.balance(merchant_id, :merchant_available, :BRL)).to eq(-100)
        expect(::Stern.balance(merchant_id, :chargeback_fee_merchant, :BRL)).to eq(100)
      end

      it "writes one entry pair (charge_chargeback_fee_merchant) keyed by merchant_id" do
        described_class.new(**valid_inputs(amount: 100)).call

        expect(EntryPair.last).to have_attributes(
          code: "charge_chargeback_fee_merchant",
          uid: merchant_id,
          amount: 100,
      ...
      it "writes charge_chargeback_fee_partner for the partner variant" do
        described_class.new(**valid_inputs(merchant_id: nil, partner_id:, amount: 100)).call
        expect(EntryPair.last.code).to eq("charge_chargeback_fee_partner")
      end
      ...
      context "with available credit" do
        ...
        it "applies partner credit for the partner variant" do
          ...
          expect(::Stern.balance(partner_id, :chargeback_fee_partner, :BRL)).to eq(100)
        end
