...
module Stern
  RSpec.describe UnlockBalance, type: :model do
    ...
    describe "#call" do
      ...
      it "moves balance from merchant_outbound back to merchant_available" do
        seed_outbound({ merchant_id: }, amount: 5000)

        described_class.new(**valid_inputs(amount: 2000)).call

        expect(::Stern.balance(merchant_id, :merchant_outbound, :BRL)).to eq(3000)
      ...
      it "succeeds when amount equals outbound balance" do
        ...
        expect(::Stern.balance(merchant_id, :merchant_outbound, :BRL)).to eq(0)
