  ...
  RSpec.describe OutstandingBalanceQuery, type: :model do
    ...
    describe "#call" do
      it "sums ending_balance across accounts within one currency" do
        ...
        expect(described_class.new(book_id: :merchant_available, currency: :BRL).call).to eq(150)
        expect(described_class.new(book_id: :merchant_available, currency: :USD).call).to eq(999)
      end

      it "returns 0 when there are no entries in the currency" do
        seed(1, 1101, 100, brl)
        expect(described_class.new(book_id: :merchant_available, currency: :EUR).call).to eq(0)
      end
      ...
      it "uses the latest ending_balance per gid, not a sum of amounts" do
        ...
        expect(described_class.new(book_id: :merchant_available, currency: :BRL).call).to eq(80)
      end
      ...
      it "honours the timestamp cutoff per currency" do
        ...
        expect(described_class.new(book_id: :merchant_available, currency: :BRL, timestamp: cutoff).call).to eq(100)
        expect(described_class.new(book_id: :merchant_available, currency: :USD, timestamp: cutoff).call).to eq(500)
      end
    end
  end
end
